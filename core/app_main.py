                ...
                if action in ("ai_top_ev", "ai_top_prob"):
                    ...
                    def _job():
                        try:
                            ...
                            for c in (picks or [])[:max(30, min(120, len(picks) if picks else 0))]:
                                try:
                                    ...
                                    ranked.append({
                                        ...
                                        "ml_prob": p,
                                        "ev_r": ev,
                                        "adx14": float((ai_features or {}).get("adx14") or 0.0) if isinstance(ai_features, dict) else 0.0,
                                        "vol_spike": bool((ai_features or {}).get("vol_spike")) if isinstance(ai_features, dict) else False,
                                    })
                                except Exception:
                                    continue

                            # Compute a composite "quality" score to rank signals
                            # We normalize each component to ~[0,1] then combine:
                            # 40% prob + 30% EV + 20% trend strength (ADX) + 10% volume spike
                            for it in ranked:
                                try:
                                    p0 = float(it.get("ml_prob") or 0.0)
                                    p_norm = max(0.0, min(1.0, p0))
                                except Exception:
                                    p_norm = 0.0
                                try:
                                    ev0 = it.get("ev_r")
                                    ev0 = float(ev0) if ev0 is not None else None
                                except Exception:
                                    ev0 = None
                                # EV normalization: assume typical range [-1, +2]
                                if ev0 is None:
                                    ev_norm = 0.0
                                else:
                                    ev_norm = (ev0 + 1.0) / 3.0
                                    ev_norm = max(0.0, min(1.0, ev_norm))
                                try:
                                    adx0 = float(it.get("adx14") or 0.0)
                                except Exception:
                                    adx0 = 0.0
                                # ADX normalization: clamp at 50
                                trend_norm = max(0.0, min(1.0, adx0 / 50.0))
                                vol_norm = 1.0 if bool(it.get("vol_spike")) else 0.0
                                it["quality_score"] = (0.4 * p_norm) + (0.3 * ev_norm) + (0.2 * trend_norm) + (0.1 * vol_norm)
                            if not ranked:
                                _tg_send(str(chat_id), "❌ لا توجد نتائج الآن.", reply_markup=_build_menu(s))
                                return

                            if action == "ai_top_prob":
                                ranked.sort(key=lambda x: (x.get("quality_score") is None, -(x.get("quality_score") or 0.0), x["ml_prob"] is None, -(x["ml_prob"] or 0.0), -(x["ai_score"] or 0)), reverse=False)
                                ...
                                for it in ranked[:10]:
                                    ...
                                    adx=it.get("adx14")
                                    vs=it.get("vol_spike")
                                    label=f"{sym} | Q {q:.2f}" if q is not None else f"{sym} | Q ?"
                                    if p is not None:
                                        label += f" | P {p:.2f}"
                                    if adx is not None:
                                        try:
                                            label += f" | ADX {float(adx):.0f}"
                                        except Exception:
                                            pass
                                    if vs:
                                        label += " | VOL⚡"
                            ...
                            # ai_top_ev
                            ranked.sort(key=lambda x: (x.get("quality_score") is None, -(x.get("quality_score") or 0.0), x["ev_r"] is None, -(x["ev_r"] or -999.0), -(x["ml_prob"] or 0.0), -(x["ai_score"] or 0)), reverse=False)
